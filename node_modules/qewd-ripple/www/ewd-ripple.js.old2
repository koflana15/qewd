...
EWD.startEwdRipple = function() {
  ...
  EWD.on('error', function(responseObj) {
    if (toastr) toastr.error(responseObj.message);
  });

  EWD.on('info', function(responseObj) {
    if (toastr) toastr.info(responseObj.message);
  });
      ...
      EWD.send({
        ...
        if (responseObj.message.expired) {
          ...
          EWD.send({
            ...
            // force a reload to make sure we get the patients into cache
            location.reload();
          });
        }
      });
